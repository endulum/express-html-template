...
if (
  ...
  process.env.GH_CLIENT_ID.trim() !== '' &&
  process.env.GH_SECRET !== undefined &&
  process.env.GH_SECRET.trim() !== ''
) {
  passport.use(
    new GitHubStrategy(
      {
        ...
        callbackURL: `${process.env.DEPLOYMENT_URL}/github`,
      },
      async function (
        _accessToken: string,
  ...
  console.warn('Some critical env vars for GitHub authentication are missing.');
}
